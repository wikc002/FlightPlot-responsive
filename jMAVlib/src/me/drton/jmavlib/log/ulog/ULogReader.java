...
public class ULogReader extends BinaryLogReader {
    ...
    private void updateStatistics() throws IOException, FormatErrorException {
        ...
        // Build the catalog from the subscriptions actually declared by this file.
        // Multi IDs are sparse in valid ULogs, so generating 0..max creates fields
        // that do not exist and hides newly-added topics behind stale assumptions.
        Set<String> cataloguedSubscriptions = new HashSet<String>();
        for (int k = 0; k < messageSubscriptions.size(); ++k) {
            Subscription s = messageSubscriptions.get(k);
            if (s != null) {
                MessageFormat msgFormat = s.format;
                String subscriptionName = msgFormat.name + "_" + s.multiID;
                if (msgFormat.name.charAt(0) != '_' && cataloguedSubscriptions.add(subscriptionName)) {
                    for (int i = 0; i < msgFormat.fields.size(); i++) {
                        FieldFormat fieldDescr = msgFormat.fields.get(i);
                        if (!fieldDescr.name.contains("_padding")) {
                            if (fieldDescr.isArray()) {
                                for (int j = 0; j < fieldDescr.size; j++) {
                                    fieldsList.put(subscriptionName + "." + fieldDescr.name + "[" + j + "]", fieldDescr.type);
                                }
                            } else {
                                fieldsList.put(subscriptionName + "." + fieldDescr.name, fieldDescr.type);
                            }
                        }
                    }
                }
        ...
        // Keep parser notices and hardfault text for the application UI.
    }
    ...
    public String getHardfaultPlainText() {
        return hardfaultPlainText;
    }
    ...
    void applyMsgAsName(Map<String, Object> update, MessageData msg, String msg_name) {
        ...
        for (int i = 0; i < fields.size(); i++) {
            ...
            if (field.name.contains("_padding")) {
                continue;
            }
            if (field.isArray()) {
